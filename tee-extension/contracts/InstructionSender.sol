...
/// @title CredLineInstructionSender
/// @author CredLine Team (Flare Summer Signal Hackathon)
/// @notice On-chain entry point for sending credit scoring instructions to the TEE.
///         Users submit mock financial data; the TEE computes a privacy-preserving
///         credit score and returns only the numeric result.
///
/// DO NOT MODIFY: constructor, setExtensionId(), _getExtensionId()
contract CredLineInstructionSender {
    /// @notice Operation type for credit scoring actions.
    // forge-lint: disable-next-line(unsafe-typecast)
    bytes32 public constant OP_TYPE_CREDIT = bytes32("CREDIT");

    /// @notice Command for the SCORE action.
    // forge-lint: disable-next-line(unsafe-typecast)
    ...
    /// @notice Sends a credit scoring instruction to the TEE.
    /// @dev The _message should be JSON-encoded financial data:
    ///      {"userAddress":"0x...","accountAgeDays":1095,"totalTransactions":847,
    ///       "monthlyVolumeUsd":12500,"activeMonths":30}
    ///      The raw data is processed inside the TEE and discarded —
    ///      only the numeric score (300-850) is returned.
    /// @param _message JSON-encoded mock financial history payload.
    function requestScore(bytes calldata _message) external payable {
